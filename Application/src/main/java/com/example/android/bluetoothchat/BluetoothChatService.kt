...
class BluetoothChatService(private val mHandler: Handler) {
    ...
    private fun updateUserInterfaceTitle() {
        ...
        Log.d(TAG, "updateUserInterfaceTitle() $mNewState -> $threadState")
        mNewState = threadState
    ...
    fun connect(device: BluetoothDevice, secure: Boolean) {
        ...
        // Cancel any thread attempting to make a connection
        if (threadState == STATE_CONNECTING) {
            if (mConnectThread != null) {
                mConnectThread!!.cancel()
                mConnectThread = null
            }
    ...
    fun write(out: ByteArray) {
        ...
        synchronized(this) {
            if (threadState != STATE_CONNECTED) return
            r = mConnectedThread
        }
    ...
    private inner class AcceptThread(secure: Boolean) : Thread() {
        ...
        override fun run() {
            ...
            while (threadState != STATE_CONNECTED) {
                try {
